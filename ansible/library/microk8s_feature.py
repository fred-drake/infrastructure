...
def execute_process(process):
    result = subprocess.run(process, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def handle_feature(module, feature_name, feature_enabled, args):
    ...
    if not feature_exists:
        module.fail_json("Feature '{0}' does not exist in microk8s".format(feature_name))
        return
...
def main():
    ...
    try:
        handle_feature(module, feature_name=module.params['name'], feature_enabled=module.params['enabled'],
