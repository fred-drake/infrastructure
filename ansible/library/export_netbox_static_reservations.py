...
def build_reservations(token, api_prefix, subnet_dict):
    ...
    for key in subnet_dict:
        n = ipaddress.ip_network(
            f"{subnet_dict[key]['subnet']}/{subnet_dict[key]['netmask']}"
        )
    ...
    while True:
        ...
        request = urllib.request.Request(
            url="{0}?{1}".format(url, params), headers=headers
        )
        ...
        for result in response["results"]:
            ...
            if not dns_name:
                raise AppException(
                    f"IP address {address} is configured in Netbox as \
                    reserved, but has no DNS name!"
                )

            # Needs a device and interface associated with this
            # IP address in Netbox
            if not result["assigned_object"]:
                raise AppException(
                    f"IP address {address} has no device / interface assigned!"
                )
            ...
            # Has a device and interface associated with this IP address,
            # but also needs a mac address in Netbox
            if not mac_address:
                print(
                    f"IP address {address} has device / interface \
                    assigned ({intf_response['device']['display']}/\
                    {intf_response['display']}) but no mac address \
            ...
            for key in subnet_dict:
                if (
                    int(ipaddress.ip_address(address)) & netmasks[key]["mask"]
                ) == netmasks[key]["address"]:
...
def main():
    ...
    try:
        reservations = build_reservations(
            ...
            api_prefix=module.params["netbox_api_prefix"],
