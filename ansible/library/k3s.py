...
def execute_process(process, shell=False):
    result = subprocess.run(process, shell=shell, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def main():
    ...
    fields = {
        ...
        'role': {'required': False, 'default': 'master', 'choices': ['master', 'worker']},
